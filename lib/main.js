...
export const main = async ({ published, onlyChangesets, cwd = process.cwd(), } = {}) => {
    const { CI, GITLAB_HOST, GITLAB_TOKEN, HOME, NPM_TOKEN, DEBUG_GITLAB_CREDENTIAL = 'false', } = env;
    ...
    if (CI) {
        ...
        await exec('git', [
            ...
            'set-url',
            'origin',
            `${url.protocol}//${username}:${GITLAB_TOKEN}@${url.host}${url.pathname.replace(/\/$/, '')}/${env.CI_PROJECT_PATH}.git`,
