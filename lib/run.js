...
import { execWithOutput, getChangedPackages, getChangelogEntry, getOptionalInput, getVersionsByDirectory, sortTheThings, } from './utils.js';
...
export async function runPublish({ script, gitlabToken, createGitlabReleases = true, cwd = process.cwd(), }) {
    ...
    if (tool === 'root') {
        ...
        if (createGitlabReleases) {
            await Promise.all(releasedPackages.map(pkg => createRelease(api, {
                pkg,
...
const cjsRequire = typeof require === 'undefined' ? _.createRequire(import.meta.url) : require;
